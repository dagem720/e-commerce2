    ...
    <v-breadcrumbs class="pb-0" :items="breadcrums"></v-breadcrumbs>
       ...
       <v-row dense>
          ...
          <div class="row text-center">
            ...
            <div class="col-md-3 col-sm-6 col-xs-12" :key="pro.id" v-for="pro in fooditems">
        ...
        computed: {
          fooditems()
          {
            if(this.select == 'Price: High to Low'){
              return this.$store.getters.FooditemsPricehightolow.filter((item)=>{
            return item.typeofCatagory === 'Vegetables' && item.cProduct==='Food'
          })
            }
            if(this.select == 'Price: Low to High'){
              return this.$store.getters.FooditemsPricelowtohigh.filter((item)=>{
            return item.typeofCatagory === 'Vegetables' && item.cProduct==='Food'
          })
            }
            if(this.select == 'Most Sold'){
              return this.$store.getters.Fooditemsmostsold.filter((item)=>{
            return item.typeofCatagory === 'Vegetables' && item.cProduct==='Food'
          })
            }
        
           return this.$store.getters.FooditemsDate.filter((item)=>{
            return item.typeofCatagory === 'Vegetables' && item.cProduct==='Food'
          })
