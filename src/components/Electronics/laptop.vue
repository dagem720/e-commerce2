    ...
    <v-breadcrumbs class="pb-0" :items="breadcrums"></v-breadcrumbs>
       ...
       <v-row dense>
          ...
          <div class="row text-center">
            ...
            <div class="col-md-3 col-sm-6 col-xs-12" :key="pro.id" v-for="pro in elitems">
        ...
        computed: {
           elitems()
          {
            if(this.select == 'Price: High to Low'){
              return this.$store.getters.ElecitemsPricehightolow.filter((item)=>{
            return item.typeofCatagory === 'Laptops' && item.cProduct==='Electronics'
          })
            }
            if(this.select == 'Price: Low to High'){
              return this.$store.getters.ElecitemsPricelowtohigh.filter((item)=>{
            return item.typeofCatagory === 'Laptops' && item.cProduct==='Electronics'
          })
            }
            if(this.select == 'Most Sold'){
              return this.$store.getters.Elecitemsmostsold.filter((item)=>{
            return item.typeofCatagory === 'Laptops' && item.cProduct==='Electronics'
          })
            }
        
           return this.$store.getters.ElecitemsDate.filter((item)=>{
            return item.typeofCatagory === 'Laptops' && item.cProduct==='Electronics'
          })
