    ...
    <v-breadcrumbs class="pb-0" :items="breadcrums"></v-breadcrumbs>
       ...
       <v-row dense>
          ...
          <div class="row text-center">
            ...
            <div class="col-md-3 col-sm-6 col-xs-12" :key="pro.id" v-for="pro in furniitems">
        ...
        computed: {
          furniitems()
          {
            if(this.select == 'Price: High to Low'){
              return this.$store.getters.FurniitemsPricehightolow.filter((item)=>{
            return item.typeofCatagory === 'Kitchen-Cabinates' && item.cProduct==='Furnitures'
          })
            }
            if(this.select == 'Price: Low to High'){
              return this.$store.getters.FurniitemsPricelowtohigh.filter((item)=>{
            return item.typeofCatagory === 'Kitchen-Cabinates' && item.cProduct==='Furnitures'
          })
            }
            if(this.select == 'Most Sold'){
              return this.$store.getters.Furniitemsmostsold.filter((item)=>{
            return item.typeofCatagory === 'Kitchen-Cabinates' && item.cProduct==='Furnitures'
          })
            }
        
           return this.$store.getters.FurniitemsDate.filter((item)=>{
            return item.typeofCatagory === 'Kitchen-Cabinates' && item.cProduct==='Furnitures'
          })
